<template>
    <div class="w-setting">
        ...
        >
            <template #item="{element}">
                <div class="w-setting__city w-city">
                    <button class="w-city__move handle"></button>
                        {{ element.name }}, {{element.country}}
                    <button class="w-city__remove" @click="this.$emit('removeCity', element)"></button>
                </div>
            </template>
        </draggable>   
        <h4>Add Location:</h4>
        <div class="w-input-wrapper">
            ...
            />
            <button class="w-add-city" @click="pushCity"></button>
            <div 
